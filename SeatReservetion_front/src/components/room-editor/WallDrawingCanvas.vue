...
// Throttle для обновления объектов при перетаскивании
let throttleTimeout = null

const throttledUpdateObject = (objectId, updates) => {
...
// Планирование отрисовки через requestAnimationFrame
const scheduleDraw = async () => {
  if (animationFrameId.value) return
  
  animationFrameId.value = requestAnimationFrame(async () => {
    await draw()
    animationFrameId.value = null
  })
}

// Поиск объекта в позиции мыши
...
const handleMouseDown = async (e) => {
  ...
  if (e.button === 0) {
    ...
    if (props.currentTool === 'select') {
      ...
      // Используем общую функцию поиска объекта
      const foundObject = findObjectAtPosition(pos)
...
const handleMouseMove = async (e) => {
  ...
  if (isDragging.value && selectedWall.value) {
    ...
    // Обновляем координаты объекта с throttle
